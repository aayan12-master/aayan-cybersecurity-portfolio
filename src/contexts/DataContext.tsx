...
const parseYear = (yearStr: string | undefined | null): number => {
  if (!yearStr) return 0;
  ...
  if (lower.includes('present') || lower.includes('ongoing')) {
...
const compareRoadmapItems = (a: RoadmapItem, b: RoadmapItem): number => {
...
export const DataProvider = ({ children }: { children: ReactNode }) => {
  ...
  const seedDatabase = async (configsRes: any, skillsRes: any, servicesRes: any, projectsRes: any, certsRes: any, roadmapRes: any, futureRes: any, socialRes: any) => {
    ...
    try {
      ...
      if ((!roadmapRes.data || roadmapRes.data.length === 0) && DEFAULT_DATA.roadmap.length > 0) {
        ...
        const seededRoadmap = [...DEFAULT_DATA.roadmap].sort(compareRoadmapItems).map((r, idx) => ({ ...r, order: idx, created_at: new Date().toISOString() }));
        const res = await supabase.from('roadmap_items').upsert(seededRoadmap);
  ...
  const ctx: DataContextType = {
    ...
    addRoadmapItem: async (item) => {
      const newItem = { ...item, id: uid(), order: 0, created_at: new Date().toISOString() };
      const tableName = 'roadmap_items';
      try {
        const updatedList = [...data.roadmap, newItem]
          .sort(compareRoadmapItems)
          .map((r, idx) => ({ ...r, order: idx, created_at: (r as any).created_at || new Date().toISOString() }));
        update(d => ({ ...d, roadmap: updatedList }));
        const res = await supabase.from(tableName).upsert(updatedList);
        console.log(`[Supabase Write] Table: ${tableName} (Add/Sort)`, { status: res.status, error: res.error });
      } catch (err: any) {
        console.error(`[Supabase Exception] Table: ${tableName} (Add)`, err);
      }
    },
    updateRoadmapItem: async (id, item) => {
      const tableName = 'roadmap_items';
      try {
        const updatedList = data.roadmap.map(r => r.id === id ? { ...r, ...item } : r)
          .sort(compareRoadmapItems)
          .map((r, idx) => ({ ...r, order: idx, created_at: (r as any).created_at || new Date().toISOString() }));
        update(d => ({ ...d, roadmap: updatedList }));
        const res = await supabase.from(tableName).upsert(updatedList);
        console.log(`[Supabase Write] Table: ${tableName} (Update/Sort)`, { status: res.status, error: res.error });
      } catch (err: any) {
        console.error(`[Supabase Exception] Table: ${tableName} (Update)`, err);
      }
    },
    deleteRoadmapItem: async (id) => {
      const tableName = 'roadmap_items';
      try {
        const delRes = await supabase.from(tableName).delete().eq('id', id);
        console.log(`[Supabase Delete] Table: ${tableName}`, { id, status: delRes.status, error: delRes.error });

        const updatedList = data.roadmap.filter(r => r.id !== id)
          .sort(compareRoadmapItems)
          .map((r, idx) => ({ ...r, order: idx, created_at: (r as any).created_at || new Date().toISOString() }));
        update(d => ({ ...d, roadmap: updatedList }));

        if (updatedList.length > 0) {
          const upsRes = await supabase.from(tableName).upsert(updatedList);
          console.log(`[Supabase Upsert Orders] Table: ${tableName}`, { status: upsRes.status, error: upsRes.error });
        }
      } catch (err: any) {
        console.error(`[Supabase Exception] Table: ${tableName} (Delete)`, err);
      }
    ...
    reorderRoadmap: async (roadmap) => {
      const sorted = [...roadmap].sort(compareRoadmapItems)
        .map((r, idx) => ({ ...r, order: idx, created_at: (r as any).created_at || new Date().toISOString() }));
      update(d => ({ ...d, roadmap: sorted }));
      const tableName = 'roadmap_items';
      try {
        const res = await supabase.from(tableName).upsert(sorted);
        console.log(`[Supabase Write] Table: ${tableName} (Reorder/Upsert)`, { status: res.status, error: res.error });
      } catch (err: any) {
        console.error(`[Supabase Exception] Table: ${tableName} (Reorder)`, err);
      }
    },
